                   ...
                   where pp.processor = ?
                     ...
                     and (p.left_opcode isnull and p.left_last_use isnull or
                          (select (p.left_opcode isnull or
                              ...
                              and (p.left_last_use isnull or
                                   p.left_last_use = tp.last_parameter_use)
                             from triple_parameters tp inner join triples p1
                               on tp.parameter_id = p1.id
                            where tp.parent_id = triples.id
                              and tp.parameter_num = 1))       -- left param

                     and (p.right_opcode isnull and p.right_last_use isnull or
                          (select (p.right_opcode isnull or
                              ...
                              and (p.right_last_use isnull or
                                   p.right_last_use = tp2.last_parameter_use)
                             from triple_parameters tp2 inner join triples p2
                               on tp2.parameter_id = p2.id
                            where tp2.parent_id = triples.id
                              and tp2.parameter_num = 2))      -- right param
